...
PKG_PREMIUM.TOTAL_PREMIUM_3F(
   ...
   :BP,:ADLOA,:EP,  :TW,  :MED_CRI,  :OTH,  :VIP,  :TL,  :V_POLICY_FEE,  :TOT_PREM, :NO_DAYS,:C_MED_STATUS,:C_CRI_STATUS,:HRL);
...
oci_bind_by_name($sidwk, ':FUAL',$fuel_type);
oci_bind_by_name($sidwk, ':KWH',$kilotwat);
//OUTPUT VARIABLE
...
oci_bind_by_name($sidwk,':C_MED_STATUS',$medical_sts,40);
oci_bind_by_name($sidwk,':C_CRI_STATUS',$criticall_sts,40);
oci_bind_by_name($sidwk,':HRL',$hrl,40);
$no_days=$no_days-1;
$rc=oci_execute($sidwk);
...
if (!$rc) {
   ...
   $x=$err["code"]; 
    if($err["code"]){
     ...
     swal({
       title: "ERROR ",
       text: "Vehicle Overage - Please Contact HO ! ",
       icon: "error",
       button: "Close.....",
       setTimeout(function () {
        Swal.close()
